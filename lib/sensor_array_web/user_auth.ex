defmodule SensorArrayWeb.UserAuth do
  ...
  @remember_me_cookie "_sensor_array_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_in_user(conn, user, params \\ %{}) do
    ...
    conn
    |> put_session(:current_team_id, user.team_id)
  ...
  def fetch_current_scope_for_user(conn, _opts) do
    with {token, conn} <- ensure_user_token(conn),
      ...
      team_id = get_session(conn, :current_team_id) || user.team_id
      scope = %{scope | current_team_id: team_id}
  ...
  defp renew_session(conn, user)
       when is_struct(conn.assigns.current_scope) and is_struct(conn.assigns.current_scope.user) and
  ...
  the `current_scope`:
      ...
      defmodule SensorArrayWeb.PageLive do
  ...
  Or use the `live_session` of your router to invoke the on_mount callback:
      ...
      live_session :authenticated, on_mount: [{SensorArrayWeb.UserAuth, :require_authenticated}] do
  ...
  defp mount_current_scope(socket, session) do
    ...
    |> Component.assign_new(:current_scope, fn ->
      ...
      scope = Scope.for_user(user)
      # Prefer session current_team_id for future team switcher; fallback to user.team_id
      team_id = session["current_team_id"] || (user && user.team_id)
      if scope do
        %{scope | current_team_id: team_id}
      else
        nil
      end
    end)
